    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        listView.setOnItemClickListener(new ZrcListView.OnItemClickListener() {
            ...
            public void onItemClick(ZrcListView parent, View view, int position, long id) {
                Intent intent = new Intent(Module05.this, NewsDetails.class);
//                Toast.makeText(Module05.this, "" + position + "----" + list.get(position - 1).getDigest(), Toast.LENGTH_SHORT).show();
    ...
    private void loadData() {
        jsonObjectRequest = new JsonObjectRequest(Const.M05 + 1, null, new Response.Listener<JSONObject>() {
            @Override
...
//                Log.d("xinwen", jsonObject.toString()+"");
                try {
                    ...
                    if(code == 200){
                        ...
                        //先清除缓存数据
                        dbManager.delete(_SliderData.class, WhereBuilder.b("moduleType", "=", "m05"));
                       ...
                       /* Log.d("xinwen", list.size()+"-----111");
                        ...
                        Toast.makeText(Module05.this,"没有数据",Toast.LENGTH_SHORT).show();
                    }else{
...
//                Log.d("xinwen",volleyError.toString()+volleyError);
    ...
    private void loadSlider(List<_SliderData> sliderDatas) {
        ...
        for (int i = 0; i <sliderDatas.size() ; i++) {
            ...
            TextSliderView textSliderView = new TextSliderView(Module05.this);
            // initialize a SliderLayout
            textSliderView
                    .description(sliderDatas.get(i).getTitle())
                    .image(sliderDatas.get(i).getImgsrc())
                    .setScaleType(BaseSliderView.ScaleType.Fit)
                    .setOnSliderClickListener(Module05.this);
    ...
    private void loadMoreData(int index) {
        jsonObjectRequest = new JsonObjectRequest(Const.M05 + index, null, new Response.Listener<JSONObject>() {
            @Override
            public void onResponse(JSONObject jsonObject) {
                Log.d("xinwen", jsonObject.toString()+"");
                try {
                    ...
                    if(code == 200){
                        ...
                        for (_NewsData newdata: list2) {
                            list.add(newdata);
                            //缓存
                            newdata.setModuleType("m05");
                            dbManager.save(newdata);
                        }
                        myModule01Adapter.notifyDataSetChanged(list);
                        listView.setLoadMoreSuccess();
                    }else if(code == 400){
                        Toast.makeText(Module05.this,"没有更多了",Toast.LENGTH_SHORT).show();
...
//                Log.d("xinwen",volleyError.toString()+volleyError);
    ...
    private void initViews() {
        ...
        toolbar.setTitle("党史博览");
        setSupportActionBar(toolbar);
        toolbar.setNavigationOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Module05.this.finish();
            }
        });
        ...
        try {
            if(dbManager.selector(_SliderData.class).where("moduleType","=","m05").findAll()!=null) {
                if (dbManager.selector(_SliderData.class).where("moduleType", "=", "m05").findAll().size() > 0) {
                    List<_SliderData> sliderDatas = dbManager.selector(_SliderData.class).where("moduleType", "=", "m05").findAll();
                    loadSlider(sliderDatas);
//                    Toast.makeText(Module05.this, "加载了" + sliderDatas.size() + "条幻灯片缓存", Toast.LENGTH_SHORT).show();
                } else {
//                    Toast.makeText(Module05.this, "没有幻灯片缓存数据", Toast.LENGTH_SHORT).show();
                }
            }
            if(dbManager.selector(_NewsData.class).where("moduleType","=","m05").findAll()!=null) {
                if (dbManager.selector(_NewsData.class).where("moduleType", "=", "m05").findAll().size() > 0) {
                    list = dbManager.selector(_NewsData.class).where("moduleType", "=", "m05").findAll();
//                    Toast.makeText(Module05.this, "加载了" + list.size() + "条缓存", Toast.LENGTH_SHORT).show();
                } else {
//                    Toast.makeText(Module05.this, "没有缓存数据", Toast.LENGTH_SHORT).show();
                }
            }
        } catch (DbException e) {
            e.printStackTrace();
